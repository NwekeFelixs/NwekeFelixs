...
$connection_mysql = mysqli_connect("localhost","root","","requital");
  ...
  @$ref = $_GET['ref'];
	  $result = mysqli_query($connection_mysql, "SELECT id FROM user WHERE referral_code='$ref'");
  ...
  $refinfo = mysqli_query($connection_mysql, "SELECT * FROM user WHERE id='$ref'");
...
$e_check = mysqli_query($connection_mysql, "SELECT email FROM user WHERE email='$em'");
...
$pswd = md5($pswd);
$pswd2 = md5($pswd2);
$query = mysqli_query($connection_mysql, "INSERT INTO user(id, nm, phone, pass, sig, refc, ref) VALUES ('','$n','$em','$ph','$pswd','$d','$refcode','$ref')");
...
// Send email 
mail($refemail, $subject2, $htmlContent2, $headers2);
